...
# Gunakan st.cache_resource untuk memuat model dan encoders hanya sekali
# Ini juga akan membantu mengisolasi masalah jika ada pemanggilan tersembunyi
@st.cache_resource
def load_trained_assets():
    try:
        ...
        st.error("Pastikan file 'prediksi_kelulusan.pkl' dan 'encoders.pkl' berada di direktori yang sama dengan 'app.py'.")
        st.stop()
    except Exception as e:
        st.error(f"Terjadi error saat memuat model atau encoders: {e}")
        st.stop()

# Panggil fungsi untuk memuat model dan encoders
model, encoders = load_trained_assets()
